...
# Native terraform writes to stderr; Windows PowerShell 5.1 + $ErrorActionPreference=Stop
# treats that as a terminating NativeCommandError even when the command succeeded.
function Get-TerraformOutputRaw {
    param([string]$Name)
    $prev = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    try {
        $text = & terraform output -raw $Name 2>$null
        if ($LASTEXITCODE -ne 0) {
            return ""
        }
        return [string]$text
...
function Invoke-GenerateOpenAPI {
    ...
    $apiBaseUrl = Get-TerraformOutputRaw -Name function_api_base_url
    ...
    $openApiTemplatePath = Join-Path $PSScriptRoot "..\..\foundry\openapi-servicing.json"
    if (-not (Test-Path $openApiTemplatePath)) {
        Write-Error-Custom "OpenAPI template not found: $openApiTemplatePath"
        exit 1
    }
    $openApiTemplatePath = (Resolve-Path $openApiTemplatePath).Path

    # unique_suffix is not a terraform output. Prefer the Function App name tail.
    $funcAppName = Get-TerraformOutputRaw -Name function_app_name
    if ($funcAppName -match '-([a-z0-9]{4,8})$') {
        $suffix = $Matches[1]
    }
    else {
        $suffix = "deployed"
    ...
    # Targeted URL replace keeps the rest of the spec intact (ConvertTo-Json would reshape it)
    $openApiText = Get-Content $openApiTemplatePath -Raw
    $openApiText = [regex]::Replace(
        $openApiText,
    ...
    $outputPath = Join-Path $PSScriptRoot "..\..\foundry\openapi-servicing-$suffix.json"
    Set-Content -Path $outputPath -Value $openApiText -Encoding UTF8
